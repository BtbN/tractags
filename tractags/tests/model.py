...
from ..model import resource_tags, tag_resource, tagged_resources
...
class TagModelTestCase(unittest.TestCase):
    ...
    def test_get_tags(self):
        ...
        self.assertEqual([tag for tag in resource_tags(self.env, resource)],
                         ['tag1'])
    ...
    def test_reparent(self):
        ...
        self.assertEqual(dict(TaggedPage=set(['tag1'])), self._tags())
    ...
    def test_tag_changes(self):
        ...
        self.assertEqual(dict(TaggedPage=tags, WikiStart=tags), self._tags())
        # Add new tag to already tagged resource.
        resource = Resource(self.realm, 'WikiStart')
        tags = set(['tag1', 'tag2'])
        tag_resource(self.env, resource, author=self.req.authname, tags=tags)
        self.assertEqual(dict(TaggedPage=set(['tag1']), WikiStart=tags),
                         self._tags())
...
def test_suite():
    ...
    suite.addTest(makeSuite(TagModelTestCase))
    return suite
